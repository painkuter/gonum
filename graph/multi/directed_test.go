...
func directedBuilder(nodes []graph.Node, edges []graph.WeightedEdge, _, _ float64) (g graph.Graph, n []graph.Node, e []graph.Edge, s, a float64, ok bool) {
	seen := make(set.Nodes)
	...
	for _, edge := range edges {
		...
		cl := multi.Line{F: f, T: t, UID: dg.NewLine(f, t).ID()}
		seen.Add(cl.F)
		seen.Add(cl.T)
		e = append(e, cl)
		dg.SetLine(cl)
