...
func weightedDirectedBuilder(nodes []graph.Node, edges []graph.WeightedEdge, self, absent float64) (g graph.Graph, n []graph.Node, e []graph.Edge, s, a float64, ok bool) {
	seen := make(set.Nodes)
	...
	for _, edge := range edges {
		...
		cl := multi.WeightedLine{F: f, T: t, UID: dg.NewWeightedLine(f, t, 0).ID(), W: edge.Weight()}
		seen.Add(cl.F)
		seen.Add(cl.T)
		e = append(e, cl)
